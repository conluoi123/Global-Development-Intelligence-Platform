...
##  Ngữ Cảnh Hệ Thống & Kiến Trúc Tổng Quan
...
- **Data Layer (Tầng Dữ Liệu):** Tiêu thụ dữ liệu từ World Bank (JSON) một cách an toàn (Idempotent) vào tầng Bronze, làm sạch tại tầng Silver (CSV/Delta), và tổng hợp thành các Feature Store tại tầng Gold.
  ...
  ![Complete Data Pipeline](docs/complete_data_pipeline.png)
...
##  Điều Phối Luồng Công Việc (Airflow Workflow Orchestration)

Toàn bộ quá trình thu thập dữ liệu, xử lý thô, trích xuất đặc trưng và suy luận AI được tự động hóa và điều phối thông qua Apache Airflow (triển khai trên Docker).

![Airflow Overview](docs/overview_airflow.png)

### 1. Luồng di chuyển dữ liệu (Data Pipeline Flow):
Chi tiết quá trình biến đổi dữ liệu qua các tầng Bronze, Silver và Gold:
![Data Pipeline Flow](docs/data_pipeline_flow.png)

### 2. Luồng suy luận AI & Dự báo vĩ mô (AI Inference & Forecasting Pipeline Graph):
DAG `gdip_ml_inference` thực hiện việc chạy dự đoán rủi ro (predict_risk_next_year) và dự báo chuỗi thời gian 5 năm (forecast_macro_5y) song song:
![Airflow ML Inference Graph](docs/pipeline_ml_inference.png)
